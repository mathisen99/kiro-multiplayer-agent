...
- [x] 1. Establish the exact pinned foundation and validation commands
  - [x] 1.1 Scaffold or adapt the existing repository to the fixed Next.js App Router, React, TypeScript, and Tailwind structure
  ...
  - [x] 1.2 Configure runtime examples, ignored files, and the required validation scripts
...
- [x] 2. Implement the SQLite schema, trusted data helper, and shared contracts
  - [x] 2.1 Create the exact four-table SQLite schema and process-wide database connection
  ...
  - [x] 2.2 Define compact shared validation, snapshot, sorting, and Safe Error contracts
...
- [x] 3. Add concise Kiro guidance and demo-readiness artifacts
  - [x] 3.1 Create always-included and file-matched Steering files
  ...
  - [x] 3.2 Create the post-task quality Hook, demo-readiness Skill, and honest checklist
...
- [x] 4. Deliver room creation, joining, snapshots, and the real presentable board vertical slice
  - [x] 4.1 Implement atomic room creation, room retrieval, and nickname-only joining routes
  ...
  - [x] 4.2 Build the home flow, joined-state gate, room snapshot shell, and board-first layout
...
- [x] 5. Add server-authoritative human card creation and editing
  - [x] 5.1 Implement human card create and edit route handlers
  ...
  - [x] 5.2 Wire human card composition and editing into the four-section board
...
- [x] 6. Implement non-overlapping polling synchronization and recovery
  - [x] 6.1 Add the room polling state machine and connect all local mutations
...
- [x] 7. Implement Product Agent invitation and one bounded structured run
  - [x] 7.1 Add invitation and trusted Product Agent execution routes
  ...
  - [x] 7.2 Add explicit invite/run controls and structured proposal cards
...
- [x] 8. Preserve human authority with guarded proposal approval and rejection
  - [x] 8.1 Implement proposal review route and board controls
...
- [x] 9. Generate, store, preview, copy, and download the final eligible artifact
  - [x] 9.1 Implement trusted finalization and atomic artifact replacement
  ...
  - [x] 9.2 Implement resilient stored-Markdown preview and lossless export
...
- [x] 10. Harden only the required demo path and close the validation gate
  - [x] 10.1 Finish demo-blocking states, README warning, and scope audit
  ...
  - [x] 10.2 Run `pnpm run verify` and fix only submission blockers
  ...
  - [x] 10.3 Execute the exact two-browser Required Demo Journey once
    ...
    - **Waiver record:** Completion was explicitly accepted by the user without running the interactive two-browser journey; no smoke-test pass is claimed.
    - _Requirements: 2.1-2.6, 3.1-3.4, 4.1-4.7, 5.1-5.10, 6.1-6.5, 7.1-7.8, 10.6; Required Demo Journey_
  - [x] 10.4 Update implementation task checkboxes and stop at the MVP cut line
